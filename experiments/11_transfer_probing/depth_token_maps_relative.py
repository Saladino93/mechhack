...
def resample_relative_bins(arr: np.ndarray, n_bins: int) -> tuple[np.ndarray, np.ndarray]:
    """Average an LxT token map into L x n_bins relative-position bins.

    Bin 0 corresponds to the start of the selected span and bin n_bins-1
    corresponds to the end. This makes examples of different token lengths
    comparable by plotting percent-through-input rather than absolute token id.
    """
    n_bins = max(int(n_bins), 1)
    if arr.size == 0:
        return (
            np.full((arr.shape[0] if arr.ndim else 0, n_bins), np.nan, dtype=np.float32),
            np.zeros((arr.shape[0] if arr.ndim else 0, n_bins), dtype=bool),
        )
    L, T = arr.shape
    out = np.full((L, n_bins), np.nan, dtype=np.float32)
    valid = np.zeros((L, n_bins), dtype=bool)
    if T == 1:
        out[:, 0] = arr[:, 0]
        valid[:, 0] = True
        return out, valid

    # Assign every token to a percentile bin using its rank within the selected span.
    # This preserves all tokens and avoids interpolation artifacts from very long prompts.
...
def save_heatmap(path: Path, mat: np.ndarray, title: str, source_layers: list[int | None], token_window: str, position_mode: str = "absolute"):
    ...
    if position_mode == "relative":
        ...
        if mat.shape[1] > 1:
            tick_positions = np.linspace(0, mat.shape[1] - 1, 6)
...
def main():
    ...
    ap.add_argument("--max_tokens", type=int, default=256, help="Fixed heatmap width for absolute-position plots; <=0 uses longest observed length")
    ap.add_argument("--position_mode", choices=["absolute", "relative"], default="absolute",
                    help="absolute = token index; relative = percent-through-selected-span bins")
    ap.add_argument("--relative_bins", type=int, default=100,
                    help="Number of percent bins for --position_mode relative")
    ...
    # Absolute mode uses token indices. Relative mode uses fixed percent-through-input bins.
    ...
    for p_i, p in enumerate(paths):
        ...
        try:
            ...
            if args.position_mode == "relative":
                canvas, _ = resample_relative_bins(selected_map, heatmap_width)
                c_contrib, _ = resample_relative_bins(contrib, heatmap_width)
                c_cum, _ = resample_relative_bins(cumulative, heatmap_width)
                c_layer, _ = resample_relative_bins(layer_logit, heatmap_width)
            else:
            ...
            if args.save_per_sample:
                np.savez_compressed(
                    ...
                    relative_bins=np.asarray([heatmap_width if args.position_mode == "relative" else 0], dtype=np.int64),
    ...
    onset_xlabel = "relative-position bin" if args.position_mode == "relative" else "token index in analysis window"
    save_hist(figs_dir / "pos_token_onset_hist.png", pos_token_onsets, "positive token onset", onset_xlabel)
    save_hist(figs_dir / "neg_token_onset_hist.png", neg_token_onsets, "negative token onset", onset_xlabel)
    ...
    summary = {
        ...
        "token_window": args.token_window,
        "position_mode": args.position_mode,
        "relative_bins": int(args.relative_bins),
