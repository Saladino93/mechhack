"""Unified per-token probe trainer with one shared disk pass per layer.

Loads each layer's per-token features into RAM once (fp16, ~20 GB/layer)
then trains all probe heads × seeds off that cache. This kills the disk-I/O
bottleneck that made the lazy-load trainer take ~3 min/seed.

Heads trained:
  - multimax_simple   : per-token linear → max-over-tokens
  - multimax_kramar   : Kramár 2026 Architecture C (TransformMLP→100, 10 heads, sharp softmax)
  - attention_deepmind: single learned-query soft-attention pool

3 seeds each, 4 layers (30/40/50/60), seed-averaged predictions + bootstrap CI.
"""
...
# Borrow Kramár MultiMax from exp 16
...
class AttentionProbe(nn.Module):
    """DeepMind-style single learned query soft-attention pool."""
    def __init__(self, d):
        super().__init__()
...
PROBE_FACTORIES = {
    "multimax_simple":   lambda d: MultiMaxSimple(d),
    "multimax_kramar":   lambda d: MultiMaxProbe(d_model=d),
    "attention_deepmind":lambda d: AttentionProbe(d),
}
LRS = {  # paper-aligned defaults
    "multimax_simple":    5e-4,
    "multimax_kramar":    1e-4,
    "attention_deepmind": 5e-4,
}
...
print(f"  inner_train={len(inner_train_idx)}  inner_val={len(val_idx)}  test={len(test_idx)}")


def cache_layer_features(layer):
    """Returns (tensors_list, masks_list, d) in fp16 on CPU."""
    li = LAYER_IDXS.index(layer)
    print(f"  Caching layer L{layer:02d} per-token features into RAM...")
    t0 = time.time()
    tens, masks = [], []
    for i, sid in enumerate(ids_all):
        ex = torch.load(str(EXTRACTS / f"{sid}.pt"),
                         weights_only=False, map_location="cpu")
        tens.append(ex["residuals"][li].half())   # fp16, (N, d)
        masks.append(ex["attention_mask"].bool())
        if (i+1) % 200 == 0:
            print(f"    [{i+1}/{len(ids_all)}] cached, {time.time()-t0:.1f}s")
    d = tens[0].shape[-1]
    total = sum(t.numel() for t in tens) * 2 / 1024**3
    print(f"  layer L{layer:02d} cached: {len(tens)} samples, "
          f"{total:.1f} GB fp16, {time.time()-t0:.1f}s")
    return tens, masks, d


def pad_batch(idx_list, tens, masks, d):
    """idx_list: indices into ids_all. Returns (B, T_max, d) and (B, T_max) on DEVICE."""
...
def train_one(arch, d, tens, masks, seed, epochs=15, batch=8):
    torch.manual_seed(seed); np.random.seed(seed); random.seed(seed)
    model = PROBE_FACTORIES[arch](d).to(DEVICE)
    opt = torch.optim.AdamW(model.parameters(), lr=LRS[arch], weight_decay=1e-3)
...
def predict_test(model, d, tens, masks, batch=8):
    model.eval()
...
# --------- main loop ---------
results = {arch: {"per_layer": {}} for arch in PROBE_FACTORIES}
for L in SWEEP_LAYERS:
    tens, masks, d = cache_layer_features(L)
    for arch in PROBE_FACTORIES:
        seed_aucs = []; seed_pred = []
        for seed in SEEDS:
            t0 = time.time()
            ...
            seed_aucs.append(a)
            print(f"    {arch:20s} L{L:02d} seed{seed}: AUC {a:.4f}  {time.time()-t0:.0f}s")
        ...
        print(f"  {arch:20s} L{L:02d}: AUC {auc:.4f} [{lo:.4f},{hi:.4f}]  "
              f"seed mean±std {np.mean(seed_aucs):.4f}±{np.std(seed_aucs):.4f}")
    del tens, masks  # free RAM before next layer
