# Experiment 15: MultiMax & Rolling Attention Probes — Implementation Spec
...
### File: `experiments/15_multimax_probe_omar/probes.py`

Implement these `nn.Module` classes:

1. **`TransformMLP`** — 2-layer ReLU MLP, d_in → d_hidden (default 100)
2. **`MultiMaxProbe`** — MLP transform + hard max per head (Eq. 9)
3. **`RollingAttentionProbe`** — MLP transform + attention-within-windows + max-over-windows (Eq. 10)
4. **`AttentionProbeKramar`** — MLP transform + standard softmax attention (Eq. 8) — as a baseline to compare against MultiMax/Rolling (the starter code's `AttentionProbe` doesn't have the MLP transform)
...
### File: `experiments/15_multimax_probe_omar/train.py`
...
### File: `experiments/15_multimax_probe_omar/notes.md`
