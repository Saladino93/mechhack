...
def main():
    ...
    cy1 = data["tasks"].get("cyber_1", {})
    if cy1.get("status") == "ok":
        chance_acc = max(cy1["n_pos"], cy1["n_neg"]) / (cy1["n_pos"] + cy1["n_neg"])
    else:
        chance_acc = 0.5

    common_subtitle = (f"balanced cyber_1 ({n_samples} samples), "
                       f"{n_folds}-fold stratified CV — band = ±1σ across folds")

    # AUC plot (with train overlay)
    fig, ax = plt.subplots(figsize=(9, 5.5))
    _plot_metric(ax, data, "auc_mean", "auc_std",
                 "ROC-AUC", f"Gemma 4-31B-it cyber_1 probe\n{common_subtitle}",
    ...
    _plot_metric(ax, data, "acc_mean", "acc_std",
                 "Accuracy", f"Gemma 4-31B-it cyber_1 probe\n{common_subtitle}",
    ...
    _plot_metric(ax2, data, "acc_mean", "acc_std",
                 ...
                 train_key="train_acc_mean", chance_line=chance_acc)
    fig.suptitle(f"Gemma 4-31B-it cyber_1 layer sweep — {common_subtitle}")
    fig.tight_layout()
